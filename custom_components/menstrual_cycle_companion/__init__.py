...
def _async_schedule_old_domain_migration(hass: HomeAssistant) -> None:
    """Detect old menstrual_cycle_companion entries and create repair issues for each.
...
async def _async_migrate_old_domain_entry(hass: HomeAssistant, old_entry: ConfigEntry) -> None:
    """Migrate a single menstrual_cycle_companion config entry to menstrual_cycle_companion."""
    from .repairs import async_delete_migration_issue

    try:
        # If a menstrual_cycle_companion entry with the same unique_id already exists,
        ...
        # Initiate an import flow that creates a new menstrual_cycle_companion entry.
...
async def _async_load_timer_state(hass: HomeAssistant, profile: str) -> None:
    ...
    if isinstance(timer_state, dict):
        hass.states.async_set(
            f"menstrual_cycle_companion_timer.{profile}",
